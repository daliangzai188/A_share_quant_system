...
> 当前阶段：策略 B、L 已删除；当前腿序为 `D>A>M>E>C>N`，策略E当前版本为`E_CURRENT`，N为最低优先级补位腿。
> E的正式完整样本为门禁前102个候选日、门禁后82个候选日；旧50/43行只作历史子集回归。
> 当前N v4因果执行认证为166笔/5971.386963倍/最大回撤-22.3862%；其中N=24笔、N自身复利2.197548倍、胜率75%。旧174笔/9508.426795倍含历史成交打分前视、未前复权等失真，只保留审计；机械复利不是收益承诺，容量尚未认证。
