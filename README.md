...
7. **维护与恢复通知**：账户连续 2 分钟不可查询会发送维护/掉线通知；恢复通知必须同时满足“当前 daemon PID 的调度心跳正常”和“当前 PID 已成功查询账户与持仓”，避免假恢复。

Windows keeper 的独立审计日志为 `logs/win_daemon_keeper.log`，账户健康状态为
`logs/broker_health.json`。
