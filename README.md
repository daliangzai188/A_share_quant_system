...
> 当前阶段：当前真实开仓日腿序固定为 `A>C>E>D`。2026-09-02起使用
> `ACDE_CED_V12_6046_20260630`：A不变，C/E/D切换到V12正式配置。
> `A>C>E>D`同时是后续所有单策略研究、组合回放、发布认证和实盘执行的标准腿序；普通参数优化不得改腿序，也不得使用其他顺序的更高历史复利作为发布依据。
> V12三年严格时序回放窗口为`20230701~20260630`：176笔、胜率72.73%、机械复利6046.316594倍、最大回撤-24.34%、最大单笔亏损-18.51%，分腿A78/C46/E36/D16。该组合来自同窗口`STRICT_DISCOVERY`，不是独立冻结样本外验证，也不构成未来收益承诺；2026-07~08冻结前向仅10笔且新旧组合没有差异，实盘必须先小资金验证。
...
策略C旧两分支规则和历史依据见[策略C正式版说明](docs/strategy_c.md)；当前V12新增分支以`config/strategy_config.json`和正式证书为准。
