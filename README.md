...
> 当前阶段：当前真实开仓日腿序固定为 `A>C>E>D`，策略E当前版本为`E_CURRENT`。
> `A>C>E>D`同时是后续所有单策略研究、组合回放、发布认证和实盘执行的标准腿序；普通参数优化不得改腿序，也不得使用其他顺序的更高历史复利作为发布依据。
> 当前严格锚点为`20240630~20260630`；E为门禁前113个信号日、门禁后89笔候选、独立单账户实际执行74笔，旧50/43行只作历史子集回归。
> 当前四腿真实开仓日严格研究回放为136笔、1023.791244倍，分腿A42/C47/E36/D11；A/C/E为上一交易日收盘计划，三腿均无计划时D才启动盘中路径。用户已明确接受该值低于`A>E>C>D`的1164.500295倍。协议仍为`STRICT_DISCOVERY`，机械复利不是收益承诺，容量尚未认证。

系统当前核心分层、资金状态机、安全不变量、已知短板和后续优化顺序，统一记录在
[核心框架基准](docs/core_framework.md)。后续框架调整必须以该文档为起点逐项验证。
策略C本次两分支OR条件、排序、退出和严格回放依据见[策略C正式版说明](docs/strategy_c.md)。
