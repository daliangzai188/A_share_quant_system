...
> `ACDE_E_NO_TRADE_10687_20260831_V14`：A排除每日第一名下午首次封板且不回补，
> C排除炸板次数恰为1次；E按换手率高/封单比例低等权分位排序，保留V13门禁，
> 每日第一名同时命中成交额倍率1.2～2和炸板2～3次时直接空仓且不回补；
> D使用炸板率<75%、仍封首板>=20且累计触板<40的12条盘中OR画像。
> `A>C>E>D`同时是后续所有单策略研究、组合回放、发布认证和实盘执行的标准腿序；普通参数优化不得改腿序，也不得使用其他顺序的更高历史复利作为发布依据。
> V14最近三年严格时序回放窗口为`20230901~20260831`：174笔、胜率74.14%、平均单笔账户收益5.89%、机械复利10687.850628倍、最大回撤-25.53%、最大单笔亏损-17.60%，分腿A85/C52/E29/D8；E独立59笔、30.818622倍、最大回撤-9.62%。正式配置与冻结逐腿计划哈希、逐日组合账本及两次独立回放已经对齐。该规则仍属于同窗`STRICT_DISCOVERY`，不是独立冻结样本外或真实容量认证，也不构成未来收益承诺；实盘必须先模拟和小资金验证。
...
6. **QMT 连接重试**：账户未验证成功前不进入交易调度；启动门禁前3轮完整扫描。运行中偶发只读空返回先在原会话复查，真实第3次建连才直接完整扫描备用session；仍连续失败则退出当前daemon，由keeper拉起干净进程并先对账，避免退避门把恢复路径永久饿死。
...
QMT账户健康投影仍写入 `logs/broker_health.json`，但keeper不读取它。Windows/Syncthing短暂占用目标文件时使用唯一临时文件并有限重试，写入失败不阻塞交易主流程。
