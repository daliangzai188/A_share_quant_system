...
def job_morning() -> None:
    ...
    # ⑤ 策略D监控 —— 与A/C/E全日互斥。只有“无持仓、无非D正式候选、
...
D_INTRADAY_BLOCKING_BUY_ACTIONS = frozenset({
    "ALLOW_ABC_BUY_PREVIEW",
    # 旧缓存若仍出现已退役M动作，保持fail-closed并阻断D，避免把异常缓存
...
def _d_blocking_buy_actions(decisions) -> list[str]:
    """返回占用D资金的正式候选动作，兼容未来新增的ALLOW_*_BUY动作。"""
    ...
    for _, row in decisions.iterrows():
        ...
        if action in D_INTRADAY_BLOCKING_BUY_ACTIONS:
            blocking.add(action)
            continue
        # 未来新增策略无需改D核心名单：只要决策行本身是可执行BUY，就按正式
        # 候选处理。单纯的ALLOW_*_BUY_TODAY状态播报行没有side=BUY，不会误伤。
        if (
            action.startswith("ALLOW_")
            and "BUY" in action
            and not action.startswith("ALLOW_D_")
...
def _d_candidate_legs(decisions, blocking_actions: list[str]) -> list[str]:
    if (
        decisions is None
        or decisions.empty
        or "action" not in decisions.columns
        or "strategy_leg" not in decisions.columns
    ):
        return []
    rows = decisions[
        decisions["action"].astype(str).isin(set(blocking_actions))
    ]
    legs: set[str] = set()
    for value in rows["strategy_leg"].dropna().astype(str):
        for leg in value.upper().replace("+", "/").split("/"):
            normalized = leg.strip()
            if normalized and normalized != "D":
                legs.add(normalized)
    return sorted(legs)
...
def blocks_d_for_opening_plan(decisions) -> bool:
    ...
    if _d_blocking_buy_actions(decisions):
        return True
...
def job_strategy_d(decisions=None) -> None:
...
def _e_retry_resolved_today() -> tuple[bool, str]:
    today = today_beijing().strftime("%Y%m%d")
    return (
        _e_retry_resolved_date == today,
        _e_retry_resolution_reason if _e_retry_resolved_date == today else "",
    )
...
def _e_delayed_buy_loop(combined_orders_path, decisions) -> None:
    ...
    while True:
        ...
        if not qmt_enabled:
            ...
            _mark_e_retry_resolved("E处于模拟/未启用QMT模式，未形成实盘成交")
            return
...
def _start_e_retry_thread(combined_orders_path, decisions) -> None:
    ...
    if _e_retry_running():
        ...
        return
    resolved, reason = _e_retry_resolved_today()
    if resolved:
        logger().info("E延迟开仓今日已明确结束，跳过重复启动：%s", reason)
        return
...
def _log_close_pipeline_candidate_summary(signal_date: str) -> None:
    ...
    try:
        ...
        lines = [
            "┃━━━━━━━━━━ 收盘流水线候选产物统计 ━━━━━━━━━━",
            f"┃ 信号日：{signal_date}",
            "┃ 口径：只读取当日产物；不重算、不参与下单；执行层按A>C>E>D真实开仓日互斥",
        ]
        for leg in ("A", "C", "E", "D"):
            item = summaries[leg]
...
def snapshot_realized_strategy_equity(signal_date: str) -> None:
    ...
    try:
        ...
        if open_positions:
            logger().info(
                "全策略净值快照：账户有 %d 个未平仓头寸，跳过（有持仓时总资产含浮动盈亏，"
                "不符合已实现净值口径）。", len(open_positions)
            )
            return

        path = PROJECT_ROOT / "reports" / "account_risk_shadow" / "equity_ledger.json"
        ...
        if equity_ledger_requires_bootstrap(path):
            if not (cfg.get("broker_adapter_enabled") and cfg.get("qmt_enabled")):
                logger().warning("全策略净值账本尚未初始化且券商适配未启用，保持未就绪状态。")
                return
            with _qmt_lock:
            ...
            if bootstrap_equity <= 0:
                logger().warning("全策略净值账本初始化失败：券商总资产=%s。", bootstrap_equity)
                return
        ...
        logger().info(
            "全策略净值账本：净值%.2f元，峰值%.2f元，已实现盈亏%.2f元，"
        ...
        if snapshot.initialized_now:
            logger().warning(
                "全策略净值账本已用一次券商总资产建立基线；后续只累计本系统完整平仓盈亏，"
                "入金、出金和系统外持仓不再改变策略峰值。"
            )
...
def job_post_market(end_date: str | None = None) -> None:
    ...
    snapshot_realized_strategy_equity(target_str)
...
def _log_final_decision_summary(signal_date: str, action_date_compact: str, buy_orders: Any) -> None:
    """打印【最终结果】：先判A>C>E静态计划；三腿均无候选时才允许D盘中执行。"""
    try:
        cfg = load_json_config(PROJECT_ROOT / "config" / "config.json")
        mode_name = "A_C_E_D"
        ...
        _has_ac_plan = bool(abc_rows)
        if (not _has_ac_plan and e_sig
...
def _log_decision_chain_summary(signal_date: str) -> None:
    ...
    try:
        ...
        selected = next(
            (candidates[leg] for leg in ("A", "C", "E") if leg in candidates),
            None,
        )
        positions = [
            position
        ...
        lines = [
            ...
            "┃ 当前互斥顺序：①持仓 > ②非D候选/计划(A>C>E) > ③D盘中",
            "┃",
            "┃ 【0】券商仍有本系统策略仓？",
            f"┃   ├─ 是 → 不开新仓，确认实际清仓后再等待下一候选{path_tag if blocked else ''}",
            "┃   └─ 否",
            "┃        ↓",
            "┃ 【1】今日存在A/C/E正式候选或开仓计划？",
            "┃   ├─ 是 → 按A>C>E取首个计划执行；D全日不扫描、不记录、不下单",
            "┃   └─ 否",
            "┃        ↓",
            ...
            f"┃ ② C强势龙头：{candidate_text['C']}",
            f"┃ ③ E策略：{candidate_text['E']}",
            f"┃ ④ D盘中兜底：{'已有D策略仓，串行资金门禁生效' if d_holding else '不触发｜无候选'}",
            "┃ 账户空仓时首选："
            + (
        ...
        if blocked:
            ...
            lines.append(f"┃ ★ {day_label}A/C/E均无开仓计划")
        lines.append(bottom)
        logger().info("\n".join(lines))
...
def _notify_missed_open_window_if_needed(source: str) -> None:
    ...
    if (
        not final_buy
        or not _ordinary_open_plan_expired(final_buy, today_str)
...
def push_open_plan_notification(occasion: str) -> None:
    ...
    try:
        ...
        if fb and execution_expired:
            ...
            body = f"{label}所有收盘计划策略(A/C/E)均无符合条件标的,不开新仓。"
        if hold:
            body += f" ⚠{hold}"
...
def _log_d_status_for_signal(signal_date: str) -> None:
    try:
        ...
        if open_positions:
            ...
            logger().info("  D策略明日判断：若09:20账户空仓且A/C/E均无正式候选，才允许启动D盘中监控；能否下单取决于盘中实时过滤。")
        else:
...
def main() -> None:
    ...
    # 全策略已实现净值在启动时同步一次，避免账户级风险影子状态等到收盘才刷新。
    try:
        snapshot_realized_strategy_equity(today_beijing().strftime("%Y%m%d"))
    except Exception as e:
        log.warning("启动时全策略净值快照异常：%s", e)
    ...
    # 账户风险账本同步后再启动决策播报，确保展示状态与最新已实现权益一致。
    threading.Thread(
