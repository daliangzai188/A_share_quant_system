# -*- coding: utf-8 -*-
"""研究用：下载实盘审计199笔交易卖出日的30分钟K线（QMT xtdata行情接口）。

用途：验证"10:00(及其他时点)平仓 vs 收盘平仓"的收益差异。
在 Windows（QMT客户端在线）上运行：
    python scripts/research_exit_30m_fetch.py
只用 xtdata 行情接口，不碰交易 session，对 daemon 无影响，盘后运行即可。
输出：data/processed/research_exit_30m.csv（Mac端直接分析）。
"""
import csv
import bisect
import sys
...
def main() -> None:
    ...
    print(f"待下载 {len(pairs)} 笔卖出日的30分钟K……")

    rows = []
    fails = []
    for i, (code, day, leg) in enumerate(pairs, 1):
        try:
            xtdata.download_history_data(code, period="30m", start_time=day, end_time=day)
            data = xtdata.get_market_data_ex(
                ["open", "close", "high", "low"], [code],
                period="30m", start_time=day, end_time=day + "150000",
            )
            df = data.get(code)
    ...
    out = PROJECT_ROOT / "data" / "processed" / "research_exit_30m.csv"
