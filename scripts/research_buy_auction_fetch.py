# -*- coding: utf-8 -*-
"""研究用:拉取历史交易【买入日】开盘1分钟数据,校准开盘竞价成交额占比。

目的:ABC 流动性限仓系数不拍脑袋——竞价额 ≈ 09:31bar成交额 - 后续分钟中位数,
     得到"竞价占日成交额"真实分布,取保守分位 × 参与率上限(10%) = 限仓系数。
在 Windows(QMT在线)运行: py -3.11 scripts\\research_buy_auction_fetch.py
输出: data/processed/research_buy_auction_1m.csv
"""
import csv
import bisect
import sys
...
def main() -> None:
    ...
    for i, (code, day, leg) in enumerate(pairs, 1):
        try:
            ...
            data = xtdata.get_market_data_ex(
                ...
                period="1m", start_time=day + "093000", end_time=day + "094500",
            )
            df = data.get(code)
    ...
    out = PROJECT_ROOT / "data" / "processed" / "research_buy_auction_1m.csv"
