"""验证E2实盘规则与无前视、单账户R1回测基准是否逐票一致。

验证内容：
1. 配置固定为40条R1规则，且选股条件/排序不含未来成交和收益字段；
2. 使用与实盘相同的 ``src.strategy_e2`` 构造历史候选宇宙；
3. 对锁定的50个信号日逐日核对股票与退出规则；
4. 按当前82.5%仓位复算复利、最大回撤、胜率和单账户持仓不重叠。
...
POSITION_PCT = 0.825
EXPECTED_EQUITY_MULTIPLE = 8.456976543735616
EXPECTED_MAX_DRAWDOWN = -0.18418212640892395
...
def main() -> None:
    parser = argparse.ArgumentParser(description="验证E2实盘与50笔R1基准逐票对齐")
    ...
    daily_pick = candidates.groupby("trade_date", as_index=False).head(1).copy()
    ...
    returns = pd.to_numeric(locked["net_return"], errors="raise") * POSITION_PCT
    equity = (1 + returns).cumprod()
    drawdown = equity / equity.cummax() - 1
    occupancy_errors = verify_single_account_occupancy(locked)

    summary = pd.DataFrame(
        [
            {
                "strategy_version": "E2_R1_NO_LOOKAHEAD_SINGLE_ACCOUNT_V3",
    ...
    print("E2对齐验证通过：50/50同股、50/50同退出规则，82.5%仓位复利约8.457倍。")
