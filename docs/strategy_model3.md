...
>
> 2026-08-10更新：M因样本外仅3笔且完整组合分段回撤未全部非劣，真实下单已关闭。
> 当前发布标尺为 **135笔 / 7677.946823倍 / 最大回撤 -23.51% / 胜率 70.37%**；
> 含M的150笔/29388.980134倍只保留为研究对照，不是当前实盘场景或收益预期。

## 核心优先级逻辑

当前实盘腿序为 **D>L>A>E2>C**。D由盘中时序优先；L通过model=3基础条件后无条件优先于收盘后生成的A/E2/C。M的研究顺序仍是D>L>A>M>E2>C，但`live_order_enabled=false`，不进入真实买单。

| 情况 | 结果 |
|---|---|
| L通过基础条件 | L优先，替换A/E2/C计划 |
| L不通过或没有信号 | 按A>E2>C选择 |
| M研究信号存在 | 仅记录研究/模拟，不得真实下单 |
| 任一旧仓未确认清仓 | 禁止所有新开仓 |
