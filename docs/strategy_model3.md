...
>
> 2026-08-10更新：用户基于M自身26笔、平均收益和组合复利明确接受风险并恢复M。
> 当前发布标尺为 **150笔 / 29388.980134倍 / 最大回撤 -23.56% / 胜率68.67%**。
> M未通过全部分段回撤非劣门禁，认证状态为`PASS_WITH_RISK_ACCEPTANCE`；历史倍数
> 不是实盘收益预期，容量未认证，必须先小资金验证。

## 核心优先级逻辑

当前实盘腿序为 **D>L>A>M>E>C**。D由盘中时序优先；L通过model=3基础条件后无条件优先于收盘后生成的A/M/E/C。M只被D/L/A阻断，并已按用户明确风险接受恢复真实买单。

| 情况 | 结果 |
|---|---|
| L通过基础条件 | L优先，替换A/E/C计划 |
| L不通过或没有信号 | 按A>M>E>C选择 |
| M信号存在且D/L/A均未占用 | 通过认证、回撤与成交风控后允许真实买单 |
| 任一旧仓未确认清仓 | 禁止所有新开仓 |

这条优先级是当前实盘 mode=3 的资金占用核心：同一时间只允许一套策略占用同一资金，避免 L、A/C、E、D 之间重复开仓。仅人工退出的历史 B 仓同样阻断新仓。
