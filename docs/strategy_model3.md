...
> ACDE/D/L、82.5%仓位、串行单账户和可执行成交口径重新逐日回放。

## 当前结论

> 2026-08-18更新：旧154笔/48笔L认证因候选池污染失效。model=3框架保留，
> 但L参与开关已经关闭；当前只沿用D/A/M/E/C计划，不允许L新开仓。
...
```text
mode=1：使用当前 ACDE/D 组合实盘状态机（B已删除）。
mode=2：独立 L 龙头策略状态机。
mode=3：保留原model=3状态机外壳，但当前`l_participation_enabled=false`，实际只按D>A>M>E>C执行。
```

`l_participation_enabled=false`在读取L信号和认证前直接返回mode=1计划，因此不会产生L买单；
已有L持仓的到期卖出在该开关之前处理，不会被关闭。其他真实下单仍必须通过
`LiveOrderGateway` 的交易时间、资金、仓位、涨跌停、重复委托等风控校验。
...
最终132笔构成：A 27、C 9、D普通17、D→A 2、D→C 6、E 30、L 41。
...
>
> 2026-08-18二次更新：旧154笔/24175.186295倍因48笔L候选池错误整条作废。
> 当前按D>A>M>E>C从481个信号日重新回放，唯一发布标尺为
> **145笔 / 4445.281570倍 / 最大回撤-23.5062% / 胜率68.28% / L=0**。
> M在自然年2025存在收益劣段，认证仍为`PASS_WITH_RISK_ACCEPTANCE`；历史倍数
> 不是实盘收益预期，容量未认证，必须先小资金验证。

## 核心优先级逻辑

当前实盘腿序为 **D>A>M>E>C**。D由盘中时序优先；L只生成只读影子候选，不参与排序或占用。M只被D持仓/A计划阻断，并已按用户明确风险接受恢复真实买单。

| 情况 | 结果 |
|---|---|
| 任意L影子信号 | 只读记录，不得替换或阻断其它腿 |
| D没有占用 | 按A>M>E>C选择 |
| M信号存在且D/A均未占用 | 通过认证、回撤与成交风控后允许真实买单 |
| 任一旧仓未确认清仓 | 禁止所有新开仓 |

这条优先级是当前实盘 mode=3 的资金占用核心：同一时间只允许一套策略占用同一资金，避免A/C/E/M/D之间重复开仓。已有旧L持仓仍可正常退出，但不得新开；仅人工退出的历史B仓同样阻断新仓。
