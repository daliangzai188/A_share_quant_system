...
（端口 listening 但后台不受理登录）**。周一开盘前必然恢复。

### 兜底方案（QMT 登不上时）

- **手机 App 可交易**：到期持仓必须在到期日 14:55 前手动平仓（超期就脱离回测口径）；
- **开仓可以放弃**：买不进只是丢机会，不亏钱；
- **daemon 保持停止**：QMT 不通时它只会空转敲 QMT，且 `qmt_blocked` 状态下不会误下单。

### 待改进（非交易日做）

daemon 启动门禁在 QMT 不可用时，每 10 秒做一次**全量 session 扫描**死磕，会持续干扰
终端自身登录。应加退避：连续失败 N 轮后拉长到 60 秒 + 只试首选 session，给终端留出
干净的登录窗口。
