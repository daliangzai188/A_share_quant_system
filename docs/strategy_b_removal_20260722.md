...
策略 B 已从当前交易链路退役，当前 mode=1 由原来的 A/B/C/E/D 改为 A/C/E/D；mode=3 仍在 mode=1 与 L 之间执行既有补位/替换规则。
...
原 A+B、A+B+C、ABCDE/model=3 报告包含 B 历史交易，因此这些复利、胜率和回撤指标不能直接代表删除 B 后的当前组合。`strategy_release_validation.current_release_valid` 已设置为 `false`，在完成 A/C/D/E/L 全链路重新回放前，不得把旧指标当成当前发布结果。
...
在该持仓由用户卖出并由账户心跳同步为零以前，A、C、D、E、L 均不得占用同一资金开新仓。
...
删除 B 会释放历史上由 B 占用的交易日，后续可能由 C、E、D、L 补入，也可能空仓。因此不能用“直接从旧组合扣掉 B 收益”的方法估算新复利。
