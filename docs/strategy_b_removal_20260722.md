...
策略B已从当前交易链路退役；当前唯一正式组合为D>A>M>E>C。
...
原 A+B、A+B+C 报告包含 B 历史交易，因此这些复利、胜率和回撤指标不能直接代表删除 B 后的当前组合。`strategy_release_validation.current_release_valid` 已设置为 `false`，在完成 A/C/D/E/M 全链路重新回放前，不得把旧指标当成当前发布结果。
