...
> 2026-08-03：策略B已删除；当前model=3组合已按ACDE/D/L、82.5%仓位、
> 串行单账户和可执行成交口径重新逐日回放。

## 当前结论

> 2026-08-18更新：旧冻结组合154笔中的48笔L专项审计发现，完整涨停池下
> L2本体有效0/48、每日第一名同股0/48，并有12笔退出日不符合当前T+2规则。
> 旧认证已经失效，L停止所有新开仓，只保留信号与历史记录用于研究。

策略 L 代码仍保留，但当前不得参与实盘组合。D/A/M/E/C继续按原执行链路运行；
已有持仓的退出、撤单、POV和对账逻辑不受本开关影响。
...
```text
mode=1：继续使用当前 ACDE/D 组合实盘状态机（B已删除）。
...
当前仍保留mode=3框架，但L参与开关已关闭；状态机直接沿用mode=1的D/A/M/E/C计划。

model=3基础环境现允许`market_chain_count_bucket=3_8/8_15/15_30/gte_30`。
新增的3~8组只扩容可成交机会，不改变L2本体选股，也不放宽L替换A/C/E时的
...
L独立mode=2仍关闭；当前mode=3只在既有保护规则允许时参与ACDE/D。
```

## model=3组合内L扩容结果

L2独立认证的138笔/142.35倍用于验证L策略本体；组合内结果还要受到A/C/E优先级、
...
>
> 2026-08-18二次更新：本页上述L扩容结果只保留历史审计，不能再作为实盘依据。
> 旧154笔/24175.186295倍因48笔L候选池错误整条作废；当前L已停用，正式标尺为
> **D>A>M>E>C、145笔 / 4445.281570倍 / 最大回撤-23.5062% / L=0**。
