...
策略 D（首板打板）是在 A+B+C 框架之上叠加的盘中机会策略。

**核心逻辑**：当市场处于强势情绪时，买入当日第一次涨停（首板）且曾炸板后重新封板（multi_open）的股票，在涨停价排队买入，次日开盘集合竞价卖出。

**与 A/B/C 的关系**：D 在盘中（14:00-14:55）执行，A/B/C 在收盘后（15:10）生成信号、次日开盘执行。两者使用同一笔 80% 资金，但**时序上完全不冲突**（D 卖出 → ABC 买入，先后顺序）。
