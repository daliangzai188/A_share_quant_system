...
> 2026-07-22：策略 B 已删除。本文旧回测表中的 A+B+C 组合只保留历史审计价值；当前资金关系按 A/C/D/E/M 重新计算，历史 B 仓仅人工退出时阻断 D 新开仓。
...
当前严格串行腿序为 `D > A > M > E > C > N`；
组合胜率68.07%、平均账户收益5.92%、中位数3.08%、最大回撤-22.39%、固定初始本金
累计口径10.82倍。机械满额复投为5971.386963倍，但容量未认证，不能作为实盘收益预期。
当前认证状态为`PASS_WITH_RISK_ACCEPTANCE`，原因包括E/M/N风险接受，不是D样本门槛已通过。
...
| 15:10 | 收盘流水线 + A/C/E/M 信号（job_post_market）| 是 |
...
实际持仓确认。只要A/C/D/E/M任一旧策略仓尚未真实清空，D都跳过；打新和人工仓
...
> 接力，把该常量改回实际笔数即可恢复。当前腿序为 **D>A>M>E>C>N**；M/N按用户明确
> 风险接受进入真实下单，当前N v4标尺为166笔/5971.386963倍/-22.39%。
...
5. 替换D退出价后，完整166笔组合复利不低于5971.386963倍；
6. 完整组合最大回撤不劣于-22.386233%；
7. 同时查看成交均价代理和bar最低价压力代理，不能只看乐观结果。

> **2026-08-19基准纠正**：当前唯一口径为D>A>M>E>C>N，普通D为18笔；D认证输入
...
> 当前发布标尺必须以`reports/current_portfolio_alignment`的D>A>M>E>C>N认证为准。
...
> 2026-08-20按N v4量比后门禁重放后，当前正式标尺为166笔/5971.386963倍/-22.3862%；
> 旧174笔/9508.426795倍只保留为v2失真口径历史审计。
> 接力全关结论不受影响。）
>
