...
> 2026-07-22：策略 B 已删除。本文旧回测表中的 A+B+C 组合只保留历史审计价值；当前资金关系按 A/C/D/E/N 重新计算，历史 B 仓仅人工退出时阻断 D 新开仓。
...
当前五腿发布认证尚未通过；旧认证不得继续授权新BUY，这不代表D样本门槛已经通过。
...
| 15:10 | 收盘流水线 + A/C/E/N 信号（job_post_market）| 是 |
...
> 接力，把该常量改回实际笔数即可恢复。当前腿序为 **D>A>E>C>N**；N按用户明确
> 研究风险仍需披露；当前五腿严格发布认证尚未完成，新BUY保持fail-closed。
...
> 当前发布标尺必须以重新生成的D>A>E>C>N严格认证为准；旧组合认证已经失效。
