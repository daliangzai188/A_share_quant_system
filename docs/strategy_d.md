...
> 按D=0披露。用户于2026-09-03明确要求：历史数据缺失不得伪造，但当天
> V15因子、完整QMT一分钟路径、实时封单、真实82.5%委托金额和可靠成交概率
> `>=80%`全部通过时允许实盘开仓；任何一项失败仍拒单。精确历史重算见
> `reports/entry_alignment_fix_20260902/summary.json`。
>
...
这项历史证据缺失不再永久关闭今天可观测的实时D信号；实时成交记录将作为
2026年9月冻结前向样本单独保存，不得反向改写上述历史0笔结论。
...
若D监控在09:30以后才首次启动，先恢复同机、同日、同代码配置的有效原子检查点；
检查点因过期、代码或配置变化而不可恢复时，正式`FACTOR_UNION`模式改从QMT补取
09:30至当前全部已完成1分钟K线。系统必须先完成全市场快照扫描，用日内最高价定位
全部触板目标，再逐只验证1分钟序列无缺根、无重复、价格有效；全部成功后才恢复D。
恢复前已经错过的回封不追买，只有恢复完成后最新完成分钟新形成的回封才能进入开仓
判断。任何1分钟数据不完整仍按fail-closed拒单，严禁用午后当前快照伪造早盘路径。

每个已完成分钟必须输出一次`[D候选结果]`：明确列出本分钟新增因子合格数量、股票
代码、命中分支以及最终可开仓数量；若为零也必须明确记录为零，不能只报告触板目标数。
每次进程启动后的第一条候选结论使用WARNING级别，确保外层运行窗口可见；后续零候选
分钟使用INFO写入主日志，避免刷屏。QMT回补完成只代表数据路径可用，不代表存在合格候选。

D因子发布条件同时是实盘扫描时钟的唯一来源。系统读取所有正式分支的
`reseal_time_bucket`，从公共因子桶定义推导最后一个可能产生信号的分钟；当前V15的15个
分支均为`0930_1000`，因此10:00完成最后一次认证，10:01起不再执行全市场快照扫描或
QMT分钟线查询。未来正式发布包含下午桶时，扫描会自动延长到该桶最后一分钟；只要任一
分支未限制回封时间，就保守运行至14:55。该机制不增加、不删除候选条件，只消除正式
发布窗口结束后的无效行情负载，保证实盘信号与回测事件时钟仍由同一规则决定。

D分钟路径、逐票盘中状态和原子检查点只服务当日重启恢复。监控线程结束后内存状态
随对象释放；15:10收盘任务删除`data/state/strategy_d_intraday_checkpoint_*.json`及恢复
阻断标记，禁止跨交易日累积或继承。成交、持仓、信号CSV和运行日志属于实盘审计证据，
必须保留，不得随临时路径一起删除。
若15:10任务因关机而未执行，下次启动会补清历史日期文件；盘中重启只保留当天检查点，
15:00以后启动则当天检查点也一并清除。

上述回补只有在今日无持仓、无A/C/E及其他非D正式候选、无非D开仓计划、无在途买单
或POV时才会启动。A/C/E或未来新增策略存在正式候选时，D继续按`A>C>E>D`全日关闭；
候选有任何成交后继续按串行单仓规则阻断D。
...
全天停开。盘中重启没有可恢复状态时走上述QMT 1m严格回补；若任一行情批次缺失、
分钟K线不完整或阻断标记也无法落盘，仍按真实路径不完整fail-closed。
...
| 09:20 | 仅在空仓且无非D正式候选/开仓计划时启动D后台线程，09:30开始完整扫描 | 否（线程，复用主QMT连接） |
...
| 守护进程集成 | ✅ 09:20总门允许时启动；晚启动无有效检查点时严格回补QMT 09:30至当前1m，认证失败才fail-closed；临时检查点I/O占用可在完整扫描和新READY落盘后自动恢复 |
