...
> 2026-07-22：策略 B 已删除。本文旧回测表中的 A+B+C 组合只保留历史审计价值；当前资金关系按 A/C/D/E/L 重新计算，历史 B 仓仅人工退出时阻断 D 新开仓。
...
严格串行腿序 `D > L > A > M > E > C` 的冻结历史回放共150笔，其中实际执行D 14笔；
...
| 15:10 | 收盘流水线 + A/C/E/L/M 信号（job_post_market）| 是 |
...
实际持仓确认。只要A/C/D/E/L/M任一旧策略仓尚未真实清空，D都跳过；打新和人工仓
...
> 接力，把该常量改回实际笔数即可恢复。当前腿序为 **D>L>A>M>E>C**；M按用户明确
...
> 关掉接力后组合为 22902.02x/-24.68%，再叠加腿序重排 D>L>A>M>E>C 得到当前标尺
...
> L候选池审计后已由无L的145笔/4445.281570倍/-23.5062%正式标尺取代。
> 接力全关结论不受影响。）
>
