...
所有合格分支再按`OR`组成候选D，而不是只选唯一Top1。

当前窗口共评估34,157条支持充分条件，337条达到2%/55%，去重和删除事件覆盖冗余后
剩276条有效`if`。全部OR后信号过宽：候选D 141笔、1.096366倍，ACDE
151笔、4.774653倍，均显著低于当前D 2.026124倍及ACDE 327.726719倍。因此正式
D保持不变。

半年更新入口为`scripts/optimize_strategy_d_factor_union.py`。默认只研究；传入
`--apply`时也必须先通过D与ACDE双复利门，未通过绝不修改
