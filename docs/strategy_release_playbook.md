...
发布前验证
运行期不随意改参数
每月结束后用最新3年重新研究，预测下个月
先模拟，后小资金，再考虑扩大
```

月度研究的完整标准以
[ACDE月度数据更新、策略优化与精确回测标准流程](monthly_acde_optimization_sop.md)为准。
每次选出研究胜者后，必须先展示上月旧窗口、旧策略新窗口、新策略新窗口三组结果，
逐项说明规则、样本、收益、回撤和交易差异，然后停止等待用户审核。研究阶段不得直接
修改正式配置或提交代码；只有用户明确要求再次精准校验并在成功后提交，才进入最终
认证与提交阶段。
...
该顺序是组合层不变量，不是每次月度优化时重新搜索的参数。A作为规则较简单、
...
```text
acde_plan_jia_stop_gate_20260919（方案甲）
```

方案甲 = 下述V16组合 + 三处改动：A主池封单放宽到0.1%~1%；E在全市场涨停不超过30只时空仓；
组合层60日影子净值停手（跌破均线时A/C/E/D都不开新仓）。定义、运行方式、研究依据、
复核节奏与换规则硬门槛见 [equity_curve_stop.md](equity_curve_stop.md)。
以下V16描述仍适用于C、D与腿序；A、E以方案甲为准。
...
2. 如果开始了，已经运行了多少个交易日？
3. 最近一次策略发布验证是什么日期？
4. 当前是否要继续执行当前版本，还是准备月末滚动3年重新研究？
...
它必须复现A独立82笔、94.3984428272倍、E独立74笔、11.7037898965倍、
当前A>C依赖下C独立54笔、23.3966883341倍，以及真实开仓日A>C>E>D组合143笔、
2280.9020459698倍、分腿A53/C45/E35/D10。该证书仍标记
...
6. 如果已经运行到本月最后一个交易日更新节点，再按最新3年重新研究并生成预测下个月的新版本。
