...
原因：旧 A+B、A+B+C、ABCDE2/model=3 结果包含 B 历史交易。删除 B 会释放交易日，可能补入 C、E2、D、L，也可能空仓，不能简单从旧复利中扣减 B 收益。
...
当前该命令应明确拒绝运行，因为旧验证器仍是 A+B 历史口径。只有完成 A/C/D/E2/L 全链路逐日回放、重新定义样本外窗口和门槛后，才能建立新的发布验证。
