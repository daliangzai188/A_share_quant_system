...
### 16:28 行情超时导致的第二条重启链路

- 现场证据：账户与持仓在 17:26、17:27 均查询成功；17:28 的账户心跳随后执行
  `get_full_tick`，该调用在 35 秒后以 `sequence=92` 超时，唯一执行通道按不确定结果
  保护退出，keeper 才拉起新 daemon。该事件不是账户断开。
- 根因：`_print_account_status` 为展示“今日涨跌幅”，在每轮账户心跳里同步请求一次
  QMT 最新行情。行情展示不是账户连接验证的一部分，却与下单、撤单、持仓查询共用
  唯一串行通道；一次行情卡死就会阻塞随后真实交易调用。
- 源头修复：账户心跳现在只调用账户和持仓查询。现价继续使用券商持仓快照的
  `market_value / volume`；今日涨跌幅的昨收改从本地日线读取，当日日线已生成时取
  `pre_close`，盘中取上一交易日 `close`，并按文件修改时间缓存。文件缺失只省略该
  展示字段，绝不影响账户验证、浮盈亏、风控、开平仓或执行通道。
- 边界：开仓、平仓、POV 和 D 盘中判断所需的真实行情调用继续保留；这些交易必要
  调用若超时仍会阻断交易并恢复进程，不能拿过期或合成价格继续发单。
- Windows 现场验证：同步后的专项测试 30 项通过；17:42:08 启动新 daemon
  （PID 1532），启动门禁一次通过。至 17:47:11 已连续输出 7 条账户状态记录，
  始终只有同一 PID，`broker_health=verified`、`failure_count=0`，没有执行通道超时、
  账户断连或 keeper 重启；持仓的现价、今日涨跌幅、成本收益及计划平仓时间均保留。
