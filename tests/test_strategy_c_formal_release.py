"""锁定C_LEADER_UNION_20260630_V1的正式OR筛选和审计字段。"""
...
class StrategyCFormalReleaseTests(unittest.TestCase):
    ...
    def test_release_has_exactly_two_frozen_profiles(self) -> None:
    ...
    def test_release_audit_locks_double_compound_gate(self) -> None:
        ...
        self.assertEqual(audit["formal_release_decision"], "DUAL_COMPOUND_GATE_PASSED")
        self.assertEqual(audit["c_trade_count"], 55)
        self.assertAlmostEqual(audit["c_equity_multiple"], 23.617616094205008)
        self.assertAlmostEqual(audit["released_acde_equity_multiple"], 921.3365015462819)
        self.assertEqual(sum(audit["branch_executed_trade_counts"].values()), 55)
