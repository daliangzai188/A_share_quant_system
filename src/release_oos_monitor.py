    ...
    *,
    ...
    if compact:
        return (
            ...
            "再按整体、D/A/M/E/C/N、冻结优先级胜出组合、真实成交列出样本数、胜率、平均/中位收益、"
...
2. 判断 `D > A > M > E > C > N` 的资金优先级是否造成可重复的机会成本，而不是被一两次个案误导。
...
2. 收益与风险：分别对全部影子候选、D/A/M/E/C/N、账户空仓时冻结优先级胜出组合、实际进入实盘的候选和真实完整成交，列出样本数、胜率、平均收益、中位数收益、复利、最大回撤、盈亏比、最大盈利、最大亏损、连续亏损、费用、滑点及成交完成率。
...
5. D策略时序：D在信号日14:00后先买，A/M/E/C/N在收盘后才确定。禁止使用未来信息直接把D让位给收盘信号；若建议D切换，必须单独计算“T+1卖D再买其他腿”的真实费用、滑点、成交可行性和收益，不得只比较两只股票涨幅。
