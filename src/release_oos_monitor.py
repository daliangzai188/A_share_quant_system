...
2. 判断 `D > A > E > C` 的资金优先级是否造成可重复的机会成本，而不是被一两次个案误导。
...
5. D策略时序：D在信号日14:00后先买，A/E/C在收盘后才确定。禁止使用未来信息直接把D让位给收盘信号；若建议D切换，必须单独计算“T+1卖D再买其他腿”的真实费用、滑点、成交可行性和收益，不得只比较两只股票涨幅。
