    ...
    *,
    ...
    if compact:
        return (
            ...
            "先核对发布编号、日期范围、五腿覆盖、缺失/重复/未完成成交，禁止混入OOS起点前数据；"
            "再按整体、D/A/E/C/N、冻结优先级胜出组合、真实成交列出样本数、胜率、平均/中位收益、"
...
2. 判断 `D > A > E > C > N` 的资金优先级是否造成可重复的机会成本，而不是被一两次个案误导。
...
1. 数据质量：核对发布编号、OOS起点、日期范围、五腿覆盖率、缺失值、重复记录、未完成交易、一字涨停买不到、跌停卖出顺延；禁止混入OOS起点之前或其他发布版本的数据。
2. 收益与风险：分别对全部影子候选、D/A/E/C/N、账户空仓时冻结优先级胜出组合、实际进入实盘的候选和真实完整成交，列出样本数、胜率、平均收益、中位数收益、复利、最大回撤、盈亏比、最大盈利、最大亏损、连续亏损、费用、滑点及成交完成率。
...
5. D策略时序：D在信号日14:00后先买，A/E/C/N在收盘后才确定。禁止使用未来信息直接把D让位给收盘信号；若建议D切换，必须单独计算“T+1卖D再买其他腿”的真实费用、滑点、成交可行性和收益，不得只比较两只股票涨幅。
